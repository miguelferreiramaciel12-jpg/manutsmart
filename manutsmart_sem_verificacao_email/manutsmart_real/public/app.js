...
function bindAuthTabs() {
  $$('[data-auth-tab]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      setTitle(tab === 'login' ? 'Entrar' : 'Cadastro',
        tab === 'login' ? 'Acesse sua conta cadastrada.' : 'Crie uma conta e escolha a função.');
    });
  });
}
...
function bindAuthForms() {
  ...
  $('#registerForm').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      toast('Cadastro criado com sucesso. Agora você já pode entrar.');
